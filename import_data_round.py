...
# in class7 search for samples with 7.1+7.2+7.3 > 0.8 
...
# class_round contains labels to question: 
# "How rounded is the galaxy?" - Completely round / in between / cigarre-shaped
round_class  = class7[good_samp]
round_id = galaxy_id[good_samp]         #contains the corresponding galaxy IDs
round_ind = [i for i in range(l) if (good_samp[i] == True)] #contains the index number in the galaxy_id
round_num = len(round_ind)
print("Out of all {} samples we choose {} samples for learning our algorithms.".format(l, round_num))
# we create a new array of labels with binary values 0, 1 and 2 for hard classification
round_bin = np.zeros(round_num)
for i in range(round_num):
    round_bin[i] = np.argmax(round_class[i,:])
...
imageNames = round_id #over which images do you want to loop


# if images file doesn't already exist then make one
if not os.path.isdir("round_images"):
    os.makedirs("round_images")
...
for i in range(num_chunks):
    ...
    np.save("round_images/images{}.npy".format(i), tmpStore)
    print("complete \n")
    #print(tmpStore[1])
